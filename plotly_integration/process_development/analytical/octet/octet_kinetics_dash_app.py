...
proc_path = Path(__file__).parent
sys.path.insert(0, str(proc_path))
...
def update_main_content(exp_data):
    """Update main content area with tabs, controls, and plot area"""
    if not exp_data:
        return html.Div([
            html.H5("Please select an experiment to begin",
                   style={"text-align": "center", "color": "#6c757d", "margin-top": "50px"})
    ...
    return html.Div([
                    ...
                    dbc.Col([
                        ...
                        dbc.Checklist(
                            ...
                            options=[
                                ...
                                {"label": " Show Ka/Kdis", "value": "show_ka_kdis"},
                                {"label": " Annotate on Plot", "value": "annotate_plot"},
                ...
                dbc.Row([
                    dbc.Col([
                        ...
                        dbc.Checklist(
                            ...
                            value=["baseline_align", "reference_subtract", "apply_filter"],
                            inline=True
                        )
                    ], width=6),
                    dbc.Col([
        ...
        html.Div([
            ...
            # NEW: Kinetic Parameters Tab
            dbc.Tab(
                label="Kinetic Parameters",
                tab_id="tab-kinetics",
                children=[
                    dbc.Card([
                        dbc.CardBody([
                            html.H5("Kinetic Parameters Summary", className="mb-3"),
                            html.P("This table shows the kinetic parameters (Ka, Kd, Rmax, R²) for each antibody-concentration pair."),
                            dbc.Button(
                                "Refresh Table",
                                id="refresh-kinetics-table",
                                color="primary",
                                className="mb-3"
                            ),
                            html.Div(id="kinetics-table-container")
                        ])
                    ], style=CARD_STYLE)
                ]
            ),
        ], id="main-tabs", active_tab="tab-curves")
    ])
...
def generate_plot(n_clicks, exp_data, view_mode, selected_antibody, grid_columns, display_options,
    ...
    show_ka_kdis = "show_ka_kdis" in (display_options or [])
    annotate_plot = "annotate_plot" in (display_options or [])
    ...
    try:
        if view_mode == "grid":
            ...
            return generate_grid_plot(experiment_id, show_kd, show_ka_kdis, annotate_plot, show_all_steps, processing_settings, cols)
        else:
            if not selected_antibody:
                return html.Div("Please select an antibody", className="alert alert-warning")
            return generate_single_plot(experiment_id, selected_antibody, show_kd, show_ka_kdis, annotate_plot, show_all_steps, processing_settings)
...
def generate_grid_plot(experiment_id, show_kd, show_ka_kdis, annotate_plot, show_all_steps, processing_settings, cols=4):
    ...
    for sensor in sensors:
        ...
        if antibody not in antibody_data:
            ...
            # Get kinetic parameters - stored directly on sensor!
            if sensor.kd_m:
                antibody_data[antibody]['kd'] = sensor.kd_m
            if sensor.ka_1_ms:
                antibody_data[antibody]['ka'] = sensor.ka_1_ms
            if sensor.kdis_1_s:
                antibody_data[antibody]['kdis'] = sensor.kdis_1_s
    ...
    for ab in antibodies:
        ...
        if show_ka_kdis:
            if antibody_data[ab]['ka']:
                title += f"<br>Ka={antibody_data[ab]['ka']:.2e}1/Ms"
            if antibody_data[ab]['kdis']:
                title += f"<br>Kdis={antibody_data[ab]['kdis']:.2e}1/s"
    ...
    # Add kinetic parameter annotations on plots if requested
    if annotate_plot:
        for idx, antibody in enumerate(antibodies):
            row = (idx // cols) + 1
            col = (idx % cols) + 1
            ...
            annotation_lines = []
            if ab_data['kd']:
                annotation_lines.append(f"KD: {ab_data['kd']:.2e} M")
            if ab_data['ka']:
                annotation_lines.append(f"Ka: {ab_data['ka']:.2e} 1/Ms")
            if ab_data['kdis']:
                annotation_lines.append(f"Kdis: {ab_data['kdis']:.2e} 1/s")

            if annotation_lines:
                annotation_text = "<br>".join(annotation_lines)

                # Add text box annotation in top-right corner of subplot
                fig.add_annotation(
                    x=0.95,
                    y=0.95,
    ...
    # Create kinetics summary table for grid view
    # Get one sensor per antibody to fetch all global parameters
    table_data = []
    for antibody in sorted(antibody_data.keys()):
        # Get first sensor for this antibody to access all kinetic parameters
        first_sensor = OctetKineticsSensor.objects.filter(
            experiment_id=experiment_id,
        ...
        if first_sensor:
            table_data.append({
                'Antibody': antibody,
                'KD (M)': f"{first_sensor.kd_m:.2e}" if first_sensor.kd_m else "N/A",
                'KD Error': f"{first_sensor.kd_error:.2e}" if first_sensor.kd_error else "N/A",
                'ka (1/Ms)': f"{first_sensor.ka_1_ms:.2e}" if first_sensor.ka_1_ms else "N/A",
                'ka Error': f"{first_sensor.ka_error:.2e}" if first_sensor.ka_error else "N/A",
                'kdis (1/s)': f"{first_sensor.kdis_1_s:.2e}" if first_sensor.kdis_1_s else "N/A",
                'kdis Error': f"{first_sensor.kdis_error:.2e}" if first_sensor.kdis_error else "N/A",
                'Full R²': f"{first_sensor.r_squared:.4f}" if first_sensor.r_squared else "N/A",
                'SSG KD': f"{first_sensor.ssg_kd:.2e}" if first_sensor.ssg_kd else "N/A",
                'SSG Rmax': f"{first_sensor.ssg_rmax:.4f}" if first_sensor.ssg_rmax else "N/A",
                'SSG R²': f"{first_sensor.ssg_r_squared:.4f}" if first_sensor.ssg_r_squared else "N/A",
                'Concentrations': ', '.join([f"{c:.1f}" for c in sorted(antibody_data[antibody]['concentrations'].keys())]) + ' nM'
            })

    df_table = pd.DataFrame(table_data)

    kinetics_table = dash_table.DataTable(
    ...
    return html.Div([
        ...
        html.H5("Kinetic Parameters Summary", style={'margin-top': '30px', 'margin-bottom': '15px'}),
        kinetics_table
    ])


def generate_single_plot(experiment_id, antibody, show_kd, show_ka_kdis, annotate_plot, show_all_steps, processing_settings):
    ...
    if first_sensor.ka_1_ms:
        ka_value = first_sensor.ka_1_ms
    if first_sensor.kdis_1_s:
        kdis_value = first_sensor.kdis_1_s
    ...
    # Add kinetic parameter annotation on plot if requested
    if annotate_plot:
        annotation_lines = []
        if kd_value:
            annotation_lines.append(f"KD: {kd_value:.2e} M")
        if ka_value:
            annotation_lines.append(f"Ka: {ka_value:.2e} 1/Ms")
        if kdis_value:
            annotation_lines.append(f"Kdis: {kdis_value:.2e} 1/s")

        if annotation_lines:
            annotation_text = "<br>".join(annotation_lines)

            # Add text box annotation in top-right corner
            fig.add_annotation(
                x=0.98,
                y=0.98,
                xref="paper",
    ...
    if show_ka_kdis:
        if ka_value:
            title += f' | Ka = {ka_value:.2e} 1/Ms'
        if kdis_value:
            title += f' | Kdis = {kdis_value:.2e} 1/s'
    ...
    # Create kinetics table for single antibody view
    # Query all sensors for this antibody to get individual concentration data
    sensors = OctetKineticsSensor.objects.filter(
        experiment_id=experiment_id,
        antibody_id=antibody
    ...
    for sensor in sensors:
        table_data.append({
            'Concentration (nM)': sensor.concentration_nm,
            'Response': f"{sensor.response:.4f}" if sensor.response else "N/A",
            'KD (M)': f"{sensor.kd_m:.2e}" if sensor.kd_m else "N/A",
            'KD Error': f"{sensor.kd_error:.2e}" if sensor.kd_error else "N/A",
            'ka (1/Ms)': f"{sensor.ka_1_ms:.2e}" if sensor.ka_1_ms else "N/A",
            'ka Error': f"{sensor.ka_error:.2e}" if sensor.ka_error else "N/A",
            'kdis (1/s)': f"{sensor.kdis_1_s:.2e}" if sensor.kdis_1_s else "N/A",
            'kdis Error': f"{sensor.kdis_error:.2e}" if sensor.kdis_error else "N/A",
            'Rmax': f"{sensor.rmax:.4f}" if sensor.rmax else "N/A",
            'Rmax Error': f"{sensor.rmax_error:.4f}" if sensor.rmax_error else "N/A",
            'kobs (1/s)': f"{sensor.kobs:.2e}" if sensor.kobs else "N/A",
            'kobs Error': f"{sensor.kobs_error:.2e}" if sensor.kobs_error else "N/A",
            'Req': f"{sensor.req:.4f}" if sensor.req else "N/A",
            'Req/Rmax(%)': f"{sensor.req_rmax_percent:.2f}" if sensor.req_rmax_percent else "N/A",
            'RSS': f"{sensor.rss:.4f}" if sensor.rss else "N/A",
            'Full R²': f"{sensor.r_squared:.4f}" if sensor.r_squared else "N/A",
            'SSG KD': f"{sensor.ssg_kd:.2e}" if sensor.ssg_kd else "N/A",
            'SSG Rmax': f"{sensor.ssg_rmax:.4f}" if sensor.ssg_rmax else "N/A",
            'SSG R²': f"{sensor.ssg_r_squared:.4f}" if sensor.ssg_r_squared else "N/A",
            'Well': sensor.loading_well if sensor.loading_well else "N/A",
        })
    ...
    kinetics_table = dash_table.DataTable(
        ...
        style_data_conditional=[
            ...
            {
                'if': {
                    'filter_query': '{Concentration (nM)} = 0',
                },
    ...
    return html.Div([
        dcc.Graph(figure=fig, config={'displayModeBar': True}),
        html.H5(f"Kinetic Parameters for {antibody}", style={'margin-top': '30px', 'margin-bottom': '15px'}),
        kinetics_table
    ])


@app.callback(
    Output("kinetics-table-container", "children"),
    [Input("refresh-kinetics-table", "n_clicks"),
     Input("experiment-data", "data")]
)
def update_kinetics_table(n_clicks, exp_data):
    """Display kinetic parameters in an interactive table"""
    if not exp_data:
        return html.Div("No experiment selected", className="alert alert-info")

    experiment_id = exp_data['experiment_id']

    # Query sensors with kinetic parameters
    sensors = OctetKineticsSensor.objects.filter(
        experiment_id=experiment_id
    ).select_related('experiment').order_by('antibody_id', 'concentration_nm')
    ...
    for sensor in sensors:
        data.append({
            ...
            'SSG Rmax': f"{sensor.ssg_rmax:.4f}" if sensor.ssg_rmax else "N/A",
            'SSG R²': f"{sensor.ssg_r_squared:.4f}" if sensor.ssg_r_squared else "N/A",
            'Well': sensor.loading_well,
        })

    if not data:
        return html.Div("No kinetic parameters found", className="alert alert-info")
    ...
    table = dash_table.DataTable(
        id='kinetics-table',
        columns=[
            {'name': col, 'id': col, 'type': 'text' if col in ['Antibody', 'Well'] else 'numeric'}
    ...
    summary = html.Div([
        html.P(f"Total sensors: {len(data)}", style={"margin-top": "10px", "font-weight": "bold"}),
        html.P("Tip: Click column headers to sort, use filters, or export to Excel",
