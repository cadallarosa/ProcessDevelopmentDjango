...
from plotly_integration.models import SampleMetadata, CESDSReport
...
app = DjangoDash("CreateCESDSReportApp")


def get_default_columns_and_data():
    # Map unified table fields to display columns
    default_columns = ["sample_name", "result_id", "date_acquired", "sample_set_name"]
    # Filter for CE-SDS samples only (analysis_type = 3)
    samples = SampleMetadata.objects.filter(analysis_type=3)
    
    # Create display columns with friendly names
    column_mapping = {
    ...
    columns = [{"name": column_mapping.get(col, col.replace("_", " ").title()), "id": col} for col in default_columns]
    ...
    for sample in samples:
        ...
        for col in default_columns:
            if col == "date_acquired":
                # Map date_acquired to the old acquisition_datetime for compatibility
                row[col] = getattr(sample, "date_acquired", None)
                row["acquisition_datetime"] = row[col]  # Keep old key for compatibility
            else:
                row[col] = getattr(sample, col, None)
        
        # Also add the id field for backwards compatibility (use result_id)
        row["id"] = sample.result_id
        # Add sample_id_clean for backwards compatibility (use sample_name)
        row["sample_id_clean"] = sample.sample_name
        ...
        if "date_acquired" in row and row["date_acquired"]:
            ...
            row["acquisition_datetime"] = dt_value  # Keep for compatibility
...
app.layout = html.Div(
    ...
    children=[
        ...
        html.Div(
            children=[
                html.Label("CESDSReport Name:", style={"fontWeight": "bold"}),
                dcc.Input(
...
def update_table(sample_types, sample_set_names):
    # Map unified table columns to display columns
    selected_columns = ["result_id", "sample_name", "date_acquired", "sample_set_name"]
    display_mapping = {
        "result_id": "Result ID",
        "sample_name": "Sample Name", 
        "date_acquired": "Date Acquired",
    ...
    columns = [{"name": display_mapping.get(col, col.replace("_", " ").title()), "id": col} for col in selected_columns]

    # Filter data from unified tables - only CE-SDS samples
    query = SampleMetadata.objects.filter(analysis_type=3)
    if sample_types:
    ...
    for sample in query:
        ...
        for col in selected_columns:
            if col == "date_acquired":
                row[col] = getattr(sample, "date_acquired", None)
                row["acquisition_datetime"] = row[col]  # Keep for compatibility
            else:
                row[col] = getattr(sample, col, None)
        
        # Add compatibility fields
        row["id"] = sample.result_id
        row["sample_id_full"] = sample.sample_name
        row["sample_id_clean"] = sample.sample_name
...
#     query = CESDSMetadata.objects.all()
#     if sample_types:
#         query = query.filter(sample_prefix__in=sample_types)
#
...
def populate_sample_set_names(_):
    sample_set_names = list(
        SampleMetadata.objects.filter(analysis_type=3).values_list("sample_set_name", flat=True).distinct()
    )
...
def populate_project_ids(selected_project_id):
    ...
    project_ids = list(CESDSReport.objects.values_list("project_id", flat=True).distinct())
...
def populate_user_ids(selected_user_id):
    user_ids = CESDSReport.objects.values_list("user_id", flat=True).distinct()
...
def submit_ce_sds_report(n_clicks, report_name, project_id, new_project_id, user_id, new_user_id, comments,
                  table_data,
                  selected_rows):
    if n_clicks > 0:
        if not selected_rows:
            return "No rows selected. Please select rows to include in the CESDSReport."
        ...
        # Store CESDSReport with timestamp
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        CESDSReport.objects.create(
        ...
        return f"CESDSReport '{report_name}' created successfully with {len(sorted_samples)} samples."
