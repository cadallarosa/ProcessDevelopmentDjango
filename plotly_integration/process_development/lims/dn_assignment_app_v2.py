...
app = DjangoDash("DnAssignmentAppV2", external_stylesheets=[dbc.themes.BOOTSTRAP])
...
COLUMN_ORDER = [
    ("dn", "DN"), ("link", "Chromatogram"), ("source_material_status", "SM Status"),
    ("project_id", "Project ID"), ("unit_operation", "Unit Op"),
    ("scouting_details", "Scouting Details"), ("notes", "Notes"),
...
app.layout = html.Div([
    ...
    dbc.Modal([
        ...
        dbc.ModalBody("You have unsaved changes. Are you sure you want to leave?"),
        dbc.ModalFooter([
            dbc.Button("Cancel", id="cancel-nav", color="secondary", size="sm"),
            dbc.Button("Leave", id="confirm-nav", color="danger", size="sm")
        ])
    ], id="unsaved-modal", is_open=False),
    dbc.Modal([
        dbc.ModalHeader("Confirm Source Material Changes"),
        dbc.ModalBody(id="sm-modal-body", children="Changes detected. Do you want to overwrite the existing Source Material?"),
        dbc.ModalFooter([
            dbc.Button("Cancel", id="cancel-sm-overwrite", color="secondary", size="sm"),
            dbc.Button("Confirm Overwrite", id="confirm-sm-overwrite", color="primary", size="sm")
        ])
    ], id="sm-overwrite-modal", is_open=False),
    html.Div(id="breadcrumb-container", style={"padding": "10px", "backgroundColor": "#f8f9fa", "borderBottom": "1px solid #dee2e6"}),
    html.Div([
        dcc.Tabs(id="dn-tabs", value="select-tab", children=[
                        ...
                        columns=[
                            {
                                ...
                                "editable": key not in ("dn", "link", "date_created", "date_updated", "source_material_status")
                    ...
                    html.Div(id="dn-bulk-table-container", children=[
                        ...
                        dash_table.DataTable(
                            ...
                            columns=[
                                ...
                                for key, label in COLUMN_ORDER if key not in ("dn", "date_created", "date_updated", "id", "source_material_status", "link")
                            ],
                            data=[{col: "" for col in [
                                "dn",
                                "project_id",
            ...
            dcc.Tab(label="PD Samples", value="view-pd-tab", children=[
                html.Div([
                    ...
                    dbc.Row([
                        dbc.Col(dbc.Button("🔄 Refresh", id="refresh-pd-table", color="secondary", size="sm"),
                                width="auto"),
                        dbc.Col(dbc.Button("💾 Save Changes", id="update-pd-table", color="primary", size="sm"),
                                width="auto"),
                    ], className="mb-2"),
...
# Preload user options on app start
@app.callback(
    Output("user-options", "data"),
    Output("dn-bulk-table", "dropdown"),
    Output("edit-dn-table", "dropdown"),
    Input("dn-tabs", "value"),
    prevent_initial_call=False
)
def preload_user_options(tab):
    """Preload all usernames for dropdown menus"""
    users = User.objects.all().values_list("username", flat=True)
    user_options = [{"label": u, "value": u} for u in users]

    dropdown_config = {
        "created_by": {"options": user_options},
        "assigned_to": {"options": user_options}
    }

    return user_options, dropdown_config, dropdown_config
...
def update_breadcrumb(tab, dn_context):
    """Display breadcrumb navigation based on current context"""
    if tab == "select-tab":
        return html.Div([
            html.Span("📋 View Experiments", style={"fontWeight": "bold", "color": "#0056b3"})
        ])
    elif tab == "edit-tab":
        dn_num = dn_context.get("dn", "")
        return html.Div([
            html.Span("📋 View Experiments", style={"marginRight": "10px", "color": "#6c757d"}),
            html.Span("›", style={"marginRight": "10px", "color": "#6c757d"}),
            html.Span(f"➕ Create New DN #{dn_num}", style={"fontWeight": "bold", "color": "#0056b3"})
        ])
    elif tab == "edit-dn-tab":
        dn_num = dn_context.get("dn", "")
        return html.Div([
            html.Span("📋 View Experiments", style={"marginRight": "10px", "color": "#6c757d"}),
            html.Span("›", style={"marginRight": "10px", "color": "#6c757d"}),
            html.Span(f"✏️ Edit DN #{dn_num}", style={"fontWeight": "bold", "color": "#0056b3"}),
        ])
    elif tab == "view-pd-tab":
        return html.Div([
            html.Span("🧪 PD Samples", style={"fontWeight": "bold", "color": "#0056b3"})
        ])
    return html.Div()
...
# Inline editing save for main DN table
@app.callback(
    Output("inline-save-status", "children"),
    Output("dn-table", "data", allow_duplicate=True),
    Input("dn-table", "data"),
    State("dn-table", "data_previous"),
    prevent_initial_call=True
)
def save_inline_edits(current_data, previous_data):
    """Auto-save changes made to DN table inline"""
    if not current_data or not previous_data or current_data == previous_data:
        raise PreventUpdate

    updated = 0
    for curr_row, prev_row in zip(current_data, previous_data):
        if curr_row != prev_row:
            dn_value = curr_row.get("dn")
            if not dn_value:
                continue

            assigned_user = User.objects.filter(username=curr_row.get("assigned_to")).first()
            created_user = User.objects.filter(username=curr_row.get("created_by")).first()

            try:
                LimsDnAssignment.objects.filter(dn=dn_value).update(
                    project_id=curr_row.get("project_id", ""),
                    unit_operation=curr_row.get("unit_operation", ""),
                    scouting_details=curr_row.get("scouting_details", ""),
                    assigned_to=assigned_user,
                    created_by=created_user,
                    notes=curr_row.get("notes", ""),
                    status=curr_row.get("status", "Pending")
                )
                updated += 1
            except Exception as e:
                print(f"Error updating DN {dn_value}: {e}")

    return f"✅ Auto-saved {updated} change(s)", current_data
...
@app.callback(
    ...
    Input("dn-tabs", "value"),
    Input("inline-save-status", "children"),
    prevent_initial_call=False
)
def load_dn_table(n_clicks, tab, save_status):
    if tab != "select-tab":
        raise PreventUpdate

    assignments = LimsDnAssignment.objects.select_related("source_material").all()
    df = pd.DataFrame([{
        "dn": a.dn,
        ...
        "notes": a.notes,
        "status": a.status,
        "source_material_status": "✅ Linked" if a.source_material else "⚠️ None",
...
def auto_populate_project_id(dn_context, sample_data):
    """Auto-populate Project ID from DN context"""
    if not dn_context or dn_context.get("mode") != "edit":
        raise PreventUpdate

    dn_value = dn_context.get("dn")
    dn_instance = LimsDnAssignment.objects.filter(dn=dn_value).first()
    if not dn_instance or not sample_data:
        raise PreventUpdate

    # Auto-fill project_id for all rows
    for row in sample_data:
        if not row.get("project_id"):
            row["project_id"] = dn_instance.project_id
...
@app.callback(
    ...
    Output("sm-overwrite-modal", "is_open"),
    Output("sm-modal-body", "children"),
    Input("save-source-btn", "n_clicks"),
    Input("confirm-sm-overwrite", "n_clicks"),
    Input("cancel-sm-overwrite", "n_clicks"),
    State("sm-overwrite-modal", "is_open"),
    State("source-material-mode", "value"),
...
def save_source_material(save_clicks, confirm_clicks, cancel_clicks, modal_open, mode, selected_sm_id, name,
    ...
    if triggered_id == "cancel-sm-overwrite":
        return dash.no_update, False, dash.no_update

    if not dn_context or "dn" not in dn_context:
        return "❌ Missing DN context.", False, dash.no_update

    dn_val = dn_context["dn"]
    dn_record = LimsDnAssignment.objects.filter(dn=dn_val).first()
    if not dn_record:
        return f"❌ DN {dn_val} not found.", False, dash.no_update
    ...
    if mode == "existing" and existing_sm:
        ...
        # If save button clicked and changes detected, show modal
        if triggered_id == "save-source-btn" and changed:
            modal_message = f"Changes detected in SM{selected_sm_id}. Do you want to overwrite the existing Source Material?"
            return dash.no_update, True, modal_message

        # If confirm button clicked, proceed with overwrite
        if triggered_id == "confirm-sm-overwrite" and changed:
            ...
            dn_record.save()
        elif triggered_id == "save-source-btn":
            # ✅ Just link existing SM with no changes
            dn_record.source_material = existing_sm
            dn_record.save()
            return f"✅ Linked existing Source Material SM{selected_sm_id} to DN {dn_val} without changes.", False, dash.no_update
    ...
    return f"✅ Source Material SM{sm.sm_id} {'updated' if existing_sm and triggered_id == 'confirm-sm-overwrite' else 'created'} and linked to PD#{sm.resulting_sample.sample_id}", False, dash.no_update
