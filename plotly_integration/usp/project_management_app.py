...
Main dashboard for tracking USP experiments with bioreactors and shake flasks
"""
...
def create_gantt_chart(df, status_filter='all', harvest_filter='all', start_date=None, end_date=None):
    """Create interactive Gantt chart using USPProcessStep model"""
    print(f"Creating Gantt chart with filters: status={status_filter}, harvest={harvest_filter}")

    try:
        fig = go.Figure()

        # Define the USP workflow steps in display order
        workflow_steps = [
            {'step': 'Seed_Train', 'y_pos': 0},
            {'step': 'Bioreactor', 'y_pos': 1},
            {'step': 'Shake_Flask', 'y_pos': 2},
            {'step': 'Bioreactor_and_Shake_Flask', 'y_pos': 3}
        ]
        ...
        # For demo purposes, create dummy experiments with process steps if no real experiments exist
        if len(experiments) < 2:
            experiments = get_experiments_with_steps()
...
def get_workflow_template(template_type='standard'):
    ...
    templates = {
        'standard': [
            {'step_name': 'Seed Train Day 1', 'step_type': 'Seed_Train', 'duration': 3, 'order': 1},
            {'step_name': 'Seed Train Day 4', 'step_type': 'Seed_Train', 'duration': 3, 'order': 2},
            {'step_name': 'Bioreactor Inoculation', 'step_type': 'Bioreactor', 'duration': 14, 'order': 3},
            {'step_name': 'Shake Flask Inoculation', 'step_type': 'Shake_Flask', 'duration': 14, 'order': 4},
        ],
        'bioreactor_only': [
            {'step_name': 'Seed Train', 'step_type': 'Seed_Train', 'duration': 7, 'order': 1},
            {'step_name': 'Bioreactor Run', 'step_type': 'Bioreactor', 'duration': 14, 'order': 2},
        ],
        'shake_flask_only': [
            {'step_name': 'Seed Train', 'step_type': 'Seed_Train', 'duration': 3, 'order': 1},
            {'step_name': 'Shake Flask Run', 'step_type': 'Shake_Flask', 'duration': 14, 'order': 2},
        ],
        'parallel': [
            {'step_name': 'Seed Train', 'step_type': 'Seed_Train', 'duration': 5, 'order': 1},
            {'step_name': 'Bioreactor and Shake Flask', 'step_type': 'Bioreactor_and_Shake_Flask', 'duration': 14, 'order': 2},
        ],
...
def handle_project_modal(new_clicks, cancel_clicks, save_clicks, is_open, edit_data,
    ...
    default_values = (False, "Create New Experiment", None, "", "", date.today(),
    ...
    if new_clicks and (not cancel_clicks and not save_clicks):
        return (True, "Create New Experiment", None, "", "", date.today(),
                "Lonza", 0.0, "Minipool", None, "PROA", "Planning", ["SEC", "Titer"], "")
    ...
    if save_clicks:
        ...
        if not project_id or not sip_number or not num_samples or not transfection_start_date:
            ...
            return (is_open, "Create New Experiment", edit_data, project_id, sip_number,
