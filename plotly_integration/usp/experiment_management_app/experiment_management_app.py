...
from plotly_integration.models import USPExperiment, USPProcessStep, USPVessel, USPSeedTrain, USPMediaPrep, USPCellBank
...
app.layout = dbc.Container([
    ...
    dbc.Tabs([
        ...
        dbc.Tab(label="Seed Trains", tab_id="seed-trains-tab", children=[
            ...
            ], id="viewall-seedtrains-modal", size="xl", is_open=False),
        ]),
        ...
        dbc.Tab(label="Fed Batch", tab_id="fed-batch-tab", children=[
            ...
            ], id="viewall-fedbatch-modal", size="xl", is_open=False),
        ]),
...
def save_edit_experiment(n_clicks, exp_id, project_id, exp_name, start_date,
...
def save_all_seedtrains(n_clicks, experiment_id, step_id, project, db_ids, cell_lines, clones,
...
def populate_experiments_table(alert):
    ...
    try:
        ...
        # Create table rows with Edit buttons
        table_rows = []
        for exp in experiments:
            # Count process steps
...
def render_fedbatch_table(table_data, experiment_id):
    ...
    try:
        ...
        table_header = html.Thead(html.Tr([
            ...
            html.Th("Seed Train *", style={'min-width': '200px'}),
            html.Th("Vessel Type *", style={'min-width': '150px'}),
            html.Th("Volume (mL)", style={'min-width': '100px'}),
            html.Th("Feeding Strategy", style={'min-width': '150px'}),
            html.Th("Inoc. Date *", style={'min-width': '150px'}),
            html.Th("Inoc. Density", style={'min-width': '120px'}),
            html.Th("Harvest Date", style={'min-width': '150px'}),
            html.Th("Temp (°C)", style={'min-width': '100px'}),
            html.Th("pH", style={'min-width': '80px'}),
            html.Th("DO Probe SN", style={'min-width': '120px'}),
            html.Th("pH Probe SN", style={'min-width': '120px'}),
            html.Th("Notes", style={'min-width': '200px'}),
        ]))
...
def save_all_fedbatch(n_clicks, experiment_id, step_id, db_ids, seed_trains, vessel_types,
