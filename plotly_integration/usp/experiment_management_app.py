...
app.layout = dbc.Container([
                ...
                dbc.Tab(label="Edit Experiment", tab_id="edit-tab", children=[
                    dbc.Card([
                        dbc.CardBody([
                            ...
                            dbc.Row([
                                dbc.Col([
                                    dbc.Label("Select Experiment to Edit"),
                                    dcc.Dropdown(
                                        id="experiment-selector-dropdown",
                            ...
                            dbc.Row([
                                ...
                                dbc.Col([
                                    dbc.Label("Description"),
                                    dbc.Textarea(id="edit-exp-description", rows=2),
                                ], width=4),
                            ], className="mb-4"),
                            ...
                            dbc.Row([
                                dbc.Col([
                                    html.H5("Vessels/Bioreactors", className="mb-3"),
                                ], width=12),
                            ]),

                            # Vessel Input Section
                            dbc.Card([
                                dbc.CardHeader("Add Vessels to Process Step"),
                                dbc.CardBody([
                                    # Store for vessel data to be saved
                                    dcc.Store(id="pending-vessels-data"),
                                    dbc.Row([
                                        dbc.Col([
                                            dbc.Label("Select Process Step"),
                                            dbc.Select(
                                                id="vessel-step-select",
                                                placeholder="Select a step to add vessels..."
                                            )
                                        ], width=12),
                                    ], className="mb-3"),

                                    # Bioreactor/Shake Flask configuration (shown for non-seed train steps)
                                    html.Div([
                                        dbc.Row([
                                            dbc.Col([
                                                dbc.Label("Vessel Type/Size"),
                                                dbc.Select(
                                                    id="edit-vessel-type-select",
                                                    options=[
                                                        {"label": "2L Bioreactor", "value": "2L_BR"},
                                                        {"label": "5L Bioreactor", "value": "5L_BR"},
                                                        {"label": "10L Bioreactor", "value": "10L_BR"},
                                                        {"label": "250mL Shake Flask", "value": "250mL_SF"},
                                                        {"label": "500mL Shake Flask", "value": "500mL_SF"},
                                                        {"label": "1L Shake Flask", "value": "1L_SF"},
                                            ...
                                            dbc.Col([
                                                dbc.Label("Seed Train Source"),
                                                dbc.Select(
                                                    id="seed-train-source",
                                    ...
                                    ], id="vessel-config-section", style={"display": "none"}),

                                    # Seed Train configuration (shown for seed train steps)
                                    html.Div([
                                        dbc.Row([
                                            dbc.Col([
                                                dbc.Label("Thaw Date"),
                                                dbc.Input(
                                                    id="seed-train-thaw-date",
                                                    type="date"
                                                )
                                            ], width=3),
                                            dbc.Col([
                                        ...
                                        dbc.Row([
                                            dbc.Col([
                                                dbc.Button(
                                                    "Generate Seed Train Samples",
                                                    id="generate-seed-trains-btn",
                                                    color="primary",
                                                    className="me-2"
                                                ),
                                                dbc.Button(
                                                    "Clear",
                                                    id="clear-seed-trains-btn",
                                                    color="secondary"
                                                )
                                            ])
                                        ]),

                                        # Seed train preview table (will be editable)
                                        html.Div(id="seed-train-preview-table", className="mt-3")
                                    ], id="seed-train-config-section", style={"display": "none"}),

                                    # Preview table for vessels to be added
                                    html.Hr(className="my-3"),
                                    html.Div([
                                        html.H6("Vessels to Add:", className="mb-2"),
                                        html.Div(id="vessel-preview-container")
                                    ])
                                ])
                            ], className="mb-3"),

                            # Existing vessels display
                            html.Div(id="vessels-edit-section", className="mb-4"),
    ...
    ], id="manage-experiments-modal", size="xl", is_open=False, scrollable=True),
...
def save_experiment(save_clicks, experiment_id, experiment_name, description, start_date, status, notes, workflow_steps_data):
...
# Callback for opening manage experiments modal and loading most recent experiment
@app.callback(
...
def handle_manage_experiments_modal(manage_clicks, close_clicks, is_open):
    ...
    if button_id == "manage-experiments-btn":
        ...
        try:
            ...
            # Default values for edit form (most recent experiment)
            selected_id = None
            exp_id = ""
            exp_name = ""
            exp_status = "Planning"
            ...
            for i, exp in enumerate(experiments):
                ...
                # Load most recent experiment into edit form by default
                if i == 0:
                    selected_id = exp.id
                    exp_id = exp.experiment_id
                    exp_name = exp.experiment_name
                    exp_status = exp.status
                    exp_start = exp.start_date.strftime('%Y-%m-%d') if exp.start_date else ""
                    exp_target = exp.target_end_date.strftime('%Y-%m-%d') if exp.target_end_date else ""
                    exp_desc = exp.description or ""

            return True, table_rows, selected_id, exp_id, exp_name, exp_status, exp_start, exp_target, exp_desc
        except Exception as e:
            print(f"Error loading experiments: {e}")
            return True, [html.Tr([html.Td(f"Error: {str(e)}", colSpan=7)])], None, "", "", "", "", "", ""
...
@app.callback(
    [Output("manage-tabs", "active_tab"),
     Output("selected-experiment-id", "data", allow_duplicate=True),
     Output("edit-exp-id", "value", allow_duplicate=True),
     Output("edit-exp-name", "value", allow_duplicate=True),
     Output("edit-exp-status", "value", allow_duplicate=True),
     Output("edit-exp-start-date", "value", allow_duplicate=True),
     Output("edit-exp-target-date", "value", allow_duplicate=True),
     Output("edit-exp-description", "value", allow_duplicate=True),
     Output("process-steps-edit-section", "children"),
     Output("vessels-edit-section", "children"),
     Output("vessel-step-select", "options"),
     Output("experiment-selector-dropdown", "value", allow_duplicate=True)],
...
def handle_edit_click(n_clicks_list, current_tab):
    ...
    try:
        ...
        # Create process step options for vessel dropdown
        step_options = [
            {"label": f"Step {step.step_order}: {step.step_name} ({step.step_type.replace('_', ' ')})",
             "value": step.id}
            for step in steps
        ]
        ...
        return (
            ...
            vessels_table if table_rows else html.P("No vessels registered", className="text-muted"),
            step_options,
            experiment.id  # Set dropdown value to sync
        )
    except Exception as e:
...
def generate_vessel_preview(generate_clicks, clear_clicks, step_id, vessel_type, inoc_date,
                           vessel_count, experiment_id):
        ...
        try:
            ...
            table = dash_table.DataTable(
                ...
                columns=[
                    ...
                    {'id': 'reactor_number', 'name': 'Reactor Number', 'editable': False},
                    {'id': 'inoculation_date', 'name': 'Inoculation Date', 'editable': False}
                ],
                editable=True,
                row_deletable=True,
                dropdown={
...
def generate_seed_train_preview(generate_clicks, clear_clicks, thaw_date, sample_count, media, step_id):
        ...
        try:
            ...
            table = dash_table.DataTable(
                ...
                columns=[
                    ...
                    {'id': 'vessel_size', 'name': 'Vessel Size (mL)', 'presentation': 'dropdown'},
                    {'id': 'culture_volume', 'name': 'Culture Volume (mL)'},
                    {'id': 'thaw_date', 'name': 'Thaw Date', 'editable': False},
                    {'id': 'media', 'name': 'Media', 'editable': False}
                ],
                editable=True,
                row_deletable=True,
                dropdown={
                    'vessel_size': {
                        'options': [
                            {'label': '25', 'value': 'T25'},
                            {'label': '75', 'value': 'T75'},
                            {'label': '175', 'value': 'T175'},
                            {'label': '225', 'value': 'T225'},
                            {'label': '125', 'value': '125mL_SF'},
                            {'label': '250', 'value': '250mL_SF'},
                            {'label': '500', 'value': '500mL_SF'},
                        ]
                    }
                },
                style_cell={
...
def save_seed_trains_to_database(n_clicks, seed_train_data, step_id, experiment_id):
    ...
    try:
        ...
        for sample in seed_train_data:
            if not all([sample.get('vessel_size'), sample.get('culture_volume'), sample.get('cell_line')]):
                return (
                    dash.no_update,
                    dbc.Alert("Please fill in Vessel Size, Culture Volume, and Cell Line for all samples",
                             color="danger"),
                    True,
        ...
        for sample in seed_train_data:
            USPSeedTrain.objects.create(
                ...
                culture_volume=float(sample['culture_volume']),
...
# Callback to populate experiment dropdown when edit tab is opened
@app.callback(
    Output("experiment-selector-dropdown", "options"),
    [Input("manage-tabs", "active_tab")],
    prevent_initial_call=True
)
def populate_experiment_dropdown(active_tab):
    """Populate the experiment dropdown with all experiments"""
    if active_tab == "edit-tab":
        experiments = USPExperiment.objects.all().order_by('-created_date')
        options = [
            {
                'label': f"{exp.experiment_id} - {exp.experiment_name} ({exp.status})",
                'value': exp.id
            }
...
# Callback to load experiment when selected from dropdown
@app.callback(
    [Output("selected-experiment-id", "data", allow_duplicate=True),
     Output("edit-exp-id", "value", allow_duplicate=True),
     Output("edit-exp-name", "value", allow_duplicate=True),
     ...
     Output("process-steps-edit-section", "children", allow_duplicate=True),
     Output("vessels-edit-section", "children", allow_duplicate=True),
     Output("vessel-step-select", "options", allow_duplicate=True)],
    [Input("experiment-selector-dropdown", "value")],
    prevent_initial_call=True
)
def load_selected_experiment(experiment_id):
    """Load selected experiment into edit form"""
    ...
    try:
        ...
        ) if table_rows else html.P("No vessels registered", className="text-muted")

        # Step options for vessel registration
        step_options = [
            {'label': f"{step.step_name} ({step.step_type.replace('_', ' ')})", 'value': step.id}
            for step in steps
        ]
...
# Callback to delete process step
@app.callback(
    [Output("process-steps-edit-section", "children", allow_duplicate=True),
     Output("save-notification", "is_open", allow_duplicate=True),
     Output("save-notification", "children", allow_duplicate=True)],
...
def delete_process_step(n_clicks_list, experiment_id):
    """Delete a process step"""
    if not any(n_clicks_list) or not experiment_id:
