...
def create_summary_slides(prs, molecules, template_path):
    ...
    # Keep track of the original template slide index for cloning
    # We need to clone BEFORE modifying for the first chunk to keep template pristine
    pristine_template_idx = template_slide_idx

    for chunk_idx, chunk in enumerate(molecule_chunks, 1):
        print(f"    Summary slide {chunk_idx}: {', '.join(chunk)}")

        # Always clone the pristine template slide to avoid image overlap
        if chunk_idx == 1:
            # First chunk: clone the template and work on the clone
            # This keeps the original template pristine for subsequent clones
            slide = clone_slide(prs, pristine_template_idx)
            # After cloning, the new slide is at the end, move pristine_template_idx reference
            # Actually, we want to keep using index 0 as our pristine template
        else:
            # Additional chunks: clone the pristine template slide (index 0)
            slide = clone_slide(prs, pristine_template_idx)
